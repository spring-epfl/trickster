...
def load_transform_data_fn(path):
    ...
    padded_cells = np.array([np.pad(c, (0, max_trace_len-len(c)), 'constant')
                    for c in data])
...
def _get_normalizer():
    ...
    if not hasattr(search_params, 'normalizer'):
        search_params.normalizer = np.linalg.norm(
                search_params.clf.steps[-1][1].coef_[0],
                ord=search_params.q_norm)
...
def graph_heuristic_fn(x):
    ...
    search_params = SearchParams.get_default()
    confidence = search_params.clf.predict_proba([x.src])[
