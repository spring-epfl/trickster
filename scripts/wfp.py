...
def load_transform_data_fn(path):
    ...
    padded_cells = [np.pad(c, (0, max_trace_len-len(c)), 'constant')
                    for c in data]
    return np.array(padded_cells), labels, []
...
def clf_fit_fn(X_train, y_train, **kwargs):
    ...
    pipeline = make_pipeline(
        FunctionTransformer(
            lambda data: np.array([extract_cumul(x) for x in data])),
        clf
    )
...
def get_expansions_fn(*args, **kwargs):
    return None, None
...
class TraceNode(Node):
    def __init__(self, x, depth=0, feature_extract_fn=None):
        ...
        super().__init__(np.array(x), depth, feature_extract_fn)

    def expand(self):
        """Add dummy packets."""
        current = list(self.src)
        children = []
        for i in range(len(current) + 1):
            expanded = current[:i] + [1] + current[i+1:]
            children.append(
                    TraceNode(
                        expanded, self.depth, self.feature_extract_fn))
...
def graph_expand_fn(x):
    ...
    children = x.expand()
    costs = [np.linalg.norm(x.features - c.features,
        ord=search_params.p_norm) for c in children]
