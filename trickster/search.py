...
def hill_climbing(
    start_node, expand_fn, goal_fn, heuristic_fn=None, hash_fn=None, iter_lim=None
):
    """
    Hill climbing.
    ...
    while current_candidate_hash is not None and (
        iter_lim is None or iter_count < iter_lim
    ):

        # Retrieve the node with the lowest f-score.
        node = reverse_hashes[current_candidate_hash]

        # Check if the current node is a goal node.
        if goal_fn(node):
            return node

        closed_set.add(current_candidate_hash)
